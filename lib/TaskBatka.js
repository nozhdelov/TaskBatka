...
const InterfaceFactory = require('./Interface/Factory.js');
const OS = require('os');

class TaskBatka extends EventEmitter {
    
    static WORKER_CHILD_PROCESS = 'ChildProcess';
    static WORKER_THREAD = 'WorkerThread';
    
    constructor(options){
        super();
        options = options || {};
        this.concurrency = options.concurrency || OS.cpus().length - 1;
        this.workerType = options.workerType || TaskBatka.WORKER_CHILD_PROCESS;
        this.workers = [];
        this.taskResolvers = {};
        this.tasksQue = [];
    ...
    start(){
        ...
        for(let i = 0; i < this.concurrency; i++){
           this.workers[i] = InterfaceFactory.create(this.workerType);
    ...
    executeOnAll(task, data, method){
        ...
        return new Promise((resolve, reject) => {
           Promise.allSettled(promises).then(results => {
            resolve(results.filter(res => res.status === 'fulfilled').map(res => res.value));
           }).catch(e => reject(e)) 
        });
    }
